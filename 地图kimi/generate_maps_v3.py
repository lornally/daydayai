#!/usr/bin/env python3
"""
生成以中国为中心的世界地形图（第三批，斜轴旋转版）。
三种投影 × 两种地形风格 × 两个中心点 = 12 张图。
通过球面坐标旋转，让中国中心点位于新赤道与本初子午线交点，
从而在标准投影（Robinson, EqualEarth, Mollweide）下实现真正的x+y方向居中。
"""
...
def rotate_polygon(geom, center_lon, center_lat):
    if geom is None:
        return []
    if isinstance(geom, Polygon):
        ext = np.array(geom.exterior.coords)
        new_lon, new_lat = rotate_coords(ext[:, 0], ext[:, 1], center_lon, center_lat)
        new_lon = normalize_longitude(new_lon)
        ext_line = LineString(list(zip(new_lon, new_lat)))
        segs = split_line_at_meridian(ext_line)
        result = []
        for seg in segs:
            if len(seg.coords) >= 4:
                try:
        ...
        all_polys = []
        for g in geom.geoms:
            all_polys.extend(rotate_polygon(g, center_lon, center_lat))
        return all_polys
    else:
        return []
...
# Use ASTER GDEM Color Shaded Relief (real DEM-based 3D hillshade)
...
if not os.path.exists(base_img_path):
    ...
    url = "https://gibs.earthdata.nasa.gov/wms/epsg4326/best/wms.cgi?SERVICE=WMS&REQUEST=GetMap&LAYERS=ASTER_GDEM_Color_Shaded_Relief&VERSION=1.1.1&FORMAT=image/jpeg&SRS=EPSG:4326&BBOX=-180,-90,180,90&WIDTH=10800&HEIGHT=5400"
...
# Pre-build land masks for shaded relief
land_mask_cache = {}
for key, info in CENTERS.items():
    print(f"Pre-building land mask for {key}...")
    reader = Reader(shpreader.natural_earth(resolution='110m', category='physical', name='land'))
    land_polys = []
    for record in reader.records():
        land_polys.extend(rotate_polygon(record.geometry, info["lon"], info["lat"]))
    verts = []
    codes = []
    for poly in land_polys:
        ...
        if len(v) < 3:
            continue
        c = [MplPath.MOVETO] + [MplPath.LINETO] * (len(v) - 2) + [MplPath.CLOSEPOLY]
        verts.extend(v)
        codes.extend(c)
    land_mask_cache[key] = MplPath(verts, codes)
...
def make_map(proj_name, proj, style_name, style_config, center_key, center_info):
    ...
    if style_name == "shaded_relief":
        ax.set_facecolor(style_config["ocean_color"])
        im = ax.imshow(terrain_img, origin='upper', transform=ccrs.PlateCarree(), extent=[-180, 180, -90, 90])
        transform = ccrs.PlateCarree()._as_mpl_transform(ax)
        land_patch = PathPatch(land_mask_cache[center_key], transform=transform, facecolor='none', edgecolor='none')
        ax.add_patch(land_patch)
        im.set_clip_path(land_patch)
    else:
    ...
    filename = f"map_v3_{center_key}_{proj_name}_{style_name}.png"
